...
double run(int argc, char* argv[]){
    ...
    if (rank == 0) {
        ...
        if(ex!=-1){
            for(int i=1;i < n % size; i++) {
                MPI_Send(&A[n / size * size + i][0], n, MPI_FLOAT, i, 1, MPI_COMM_WORLD);
            }
        }
        ...
        if (ex != -1) {
            MPI_Recv(&A[ex][0], n, MPI_FLOAT, 0, 1, MPI_COMM_WORLD, &status);
        }
    }
    ...
    for (int k = 0; k < n; k++) {
        ...
        if (!ex_finish) {
            for (int j = k + 1; j < n; j++) {
                A[ex][j]=A[ex][j]-A[ex][k]*A[k][j];
            }
            B[ex]=B[ex]-A[ex][k]*B[k];
            A[ex][k]=0;
            if (ex == k + 1) {
                ex_finish = true;
            }
